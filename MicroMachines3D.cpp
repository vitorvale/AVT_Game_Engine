...
{
	pushMatrix(MODEL); // 2 identity in stack
	...
	pushMatrix(MODEL); // 2 updatebody in stack + 1 identity

	// Update Top Body Mesh
	...
	popMatrix(MODEL); // 1 UpdateTopBody in stack + 1 identity
	...
	popMatrix(MODEL); // 0 updatewheel in stack next matrix is identity
}
...
void init()
{
	...
	/*float tPos110[3] = {0.0f, -2.6f, 0.0f};
	t110 = Cheerios(&torusMesh, tPos110, gameObjectsRef.size(), 1.0f);

	t110.PrepareMeshMaterial();
	gameObjectsRef.push_back(&t110);*/
